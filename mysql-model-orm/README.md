...
- 增加
    ...
    - thenAdd(data,where): 查询后不存在则添加，{object} data，{object} where
...
第二类：辅助添加函数,一定要在主要函数前调用
    ...
    - where();条件
    - field():指定字段
    - noField()：除了table() 中 表的其他字段，不支持多表查询中的其他表的字段
    - limit():限制个数
    - group(): 分组
        ...
        // 利用trycatch捕获可能出现的错误
         try {
            ...
            //     (5.7):BETWEEN,NOT BETWEEN
                     await Model.table("tk_tab").where({id:["NOT BETWEEN",["1","3"]]}).select(); 
            ...
            // 9.join(option):string|object|array;连接查询:分为左右连接，内连接
                ...
                //(9.2)
                    await Model.table("tk_article_tab as at")
                    .join([ {table:"tk_article as a",join:"right",on:"at.a_id=a.id"},
            ...
            //15.thenAdd(data,where)://查询name为654321 不存在则添加
            await Model.table("tk_user").thenAdd({name:"123456"},{name:"654321"});
            //   return  {type:"exist",id:""} 或者 {type:"add",id:insertId};



            //16.thenUpdate(data,where)://更新id=1的字段name，如果查询name=123456 不除了自己其他不重复存在则成功更新
           ...
           //   return  {type:"exist",id:""} 或者 {type:"update",id:""}
            ...
            //还可以其他方式写sql ,传值参数方式
            // (19.1)  await transaction([{sql,value},{sql,value}])
            // (19.2)  await transaction([[sql,value],[sql,value]])



            //20：execsql(sql,option=[]);
